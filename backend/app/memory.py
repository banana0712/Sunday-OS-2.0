...
2. **避免重复提取**：如果消息内容和已有记忆高度相似，返回空数组 []
3. **用户自称的名字/昵称要记**，但必须是用户明确说"我叫XX"或"你可以叫我XX"，不要从上下文猜测
4. **不要提取模糊/不完整的信息**：
   - 错误："昵称是啥"（这不是信息，是问题）
   - 错误："用户称Sunday为…？"（不完整，有问号）
   - 错误："你想叫我啥"（这不是事实，是反问）
5. 每条记忆用一句简洁完整的话概括（不要照搬原话，不要带问号）
6. tags 是关键词标签，用于未来检索（用中文）
7. 如果一句话包含多个独立信息，拆成多条记忆
8. 如果消息不包含任何可记忆内容，返回空数组 []
9. **质量检查**：如果 summary 里有问号、省略号、或者明显是不完整的句子，不要提取
10. **昵称规则（重要！）**：
   - 标签用「昵称」，不要用具体的昵称作为标签
   - tags 示例：["昵称"] ✓，["酱酱"] ✗（不要把昵称内容放进标签）
   - 只记录用户明确告诉你的称呼方式，不要自己编造
   - 如果用户没有明确说过昵称，不要提取昵称记忆
11. **克制原则**：宁可少记，不要乱记。每封邮件最多提取2-3条记忆。
